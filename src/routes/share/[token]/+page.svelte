	...
	<div class="glass rounded-2xl p-6 text-center space-y-4">
		...
		<div class="flex justify-center gap-6 text-sm text-text-secondary">
			<span class="flex items-center gap-1.5">
				<svg class="w-4 h-4 text-neon-purple" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
					<path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/>
				</svg>
				{data.attendeeCount} attending
			</span>
			<span class="flex items-center gap-1.5">
				<svg class="w-4 h-4 text-neon-pink" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
	...
	{:else if data.isFull}
		<div class="glass rounded-2xl p-6 text-center" data-testid="party-full">
			<p class="text-text-muted text-sm font-heading">
				This party is full — no more spots available right now.
			</p>
		</div>
	{:else}
		<div class="glass rounded-2xl p-6">
			...
			{#if form?.joinError}
				<div class="mb-4 p-3 rounded-xl bg-neon-pink/10 border border-neon-pink/20 text-neon-pink text-sm font-heading" data-testid="join-error">
					{form.joinError}
				</div>
			{/if}
			...
			<form method="POST" action="?/join" use:enhance data-testid="join-form">
				<div class="flex flex-col sm:flex-row gap-3 mb-4">
					<div class="flex-1">
						<label for="join-name" class="block font-heading text-xs font-semibold text-text-secondary mb-1">Your Name</label>
						<input type="text" id="join-name" name="name" required data-testid="join-name"
					...
					<div class="flex-1">
						<label for="join-email" class="block font-heading text-xs font-semibold text-text-secondary mb-1">Your Email</label>
						<input type="email" id="join-email" name="email" required data-testid="join-email"
							class="w-full bg-surface border border-neon-purple/20 rounded-xl px-4 py-2.5 text-text-primary placeholder:text-text-muted/50 transition-colors text-sm"
							placeholder="you@email.com" />
					</div>
				</div>
				<button type="submit" data-testid="join-btn"
