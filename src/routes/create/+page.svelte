<script lang="ts">
	...
	// Auto-update maxAttendees when calculation changes, unless manually overridden
	$effect(() => {
...
<svelte:head>
	<title>Create a Party - Playlist Party</title>
</svelte:head>
...
<div class="min-h-screen flex items-start justify-center px-4 py-12 md:py-20">
	<div class="w-full max-w-lg">
		<div class="text-center mb-8">
			<a
				href="/"
				class="inline-block font-display text-2xl text-neon-pink neon-text-subtle tracking-wider mb-4"
			>
				PLAYLIST PARTY
			</a>
			<h1 class="font-heading text-3xl md:text-4xl font-bold gradient-text">Start a Party</h1>
		...
		<form method="POST" use:enhance class="glass rounded-2xl p-6 md:p-8 space-y-5">
			...
			<div class="grid grid-cols-2 gap-4">
				<div>
					<label
						for="date"
						class="block font-heading text-sm font-semibold text-text-secondary mb-1.5">Date</label
					>
					<input
						type="date"
						id="date"
				...
				<div>
					<label
						for="time"
						class="block font-heading text-sm font-semibold text-text-secondary mb-1.5">Start Time</label
					>
					<input
						type="time"
						id="time"
			...
			<div class="grid grid-cols-2 gap-4">
				<div>
					<label
						for="endTime"
						class="block font-heading text-sm font-semibold text-text-secondary mb-1.5">End Time</label
					>
					<input
						type="time"
						id="endTime"
			...
			<div class="pt-2 border-t border-neon-purple/10">
				<p
					class="font-heading text-xs font-semibold text-text-muted uppercase tracking-wider mb-4"
				>
					Party Music
				</p>
				...
				<div class="space-y-4">
					...
					<input type="hidden" name="avgSongDurationSeconds" value={avgSongDurationSeconds} />

					{#if capacityInfo}
						<p
							data-testid="guest-capacity-info"
							class="text-sm text-neon-cyan font-heading px-1"
						>
							{capacityInfo}
						</p>
					{/if}
