<script lang="ts">
	...
	const thumbnailUrl = $derived(youtubeId ? `https://img.youtube.com/vi/${youtubeId}/default.jpg` : '');
</script>
...
<div class="glass rounded-2xl px-3 py-2.5 space-y-2">
	...
	<div class="flex items-center gap-3 min-w-0">
		<!-- Thumbnail -->
		{#if thumbnailUrl}
			<img src={thumbnailUrl} alt="" class="w-[80px] h-[45px] rounded-lg object-cover flex-shrink-0 bg-surface" />
		{:else}
	...
	<div class="flex items-center gap-1.5">
		...
		<button
			onclick={ontoggle}
			class="p-2 rounded-full transition-colors {isPlaying || youtubeId
				? 'bg-neon-pink/10 text-neon-pink hover:bg-neon-pink/20'
				: 'bg-surface text-text-muted opacity-30'}"
